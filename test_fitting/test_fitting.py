...
epochs = 1_500  # The number of iterations to perform

n = float(len(points))  # Number of points

best_loss = 10_000_000_000_000.0  # Initialize with a really large value
...
for i in range(epochs):
    ...
    if new_loss < best_loss:
        print("{2}: y = {0}x + {1}   'loss = {3}'".format(m, b, i, new_loss))
        best_loss = new_loss
    else:
        m -= m_adjust
        b -= b_adjust
